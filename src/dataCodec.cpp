...
ERROR dataDecoder::_extractData() {
  ...
  if ((_buffer[0] & 0b11000000) != 0b10000000) {
    uint16_t idx = 1;
    while ((_buffer[idx] & 0b11000000) != 0b10000000) {
      idx++;
      if (idx == _bufferIndex) {
        _shiftLeftBuffer(BUFFER_SIZE);
  ...
  uint8_t len = 0;
  while ((_buffer[len + 3] & 0b11000000) != 0b11000000) {
    len++;
  ...
  if (checkSum_calc != checkSum_recv) return ERROR::INVALID_CHECKSUM;
